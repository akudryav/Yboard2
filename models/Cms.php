...
class Cms extends ActiveRecord {
    ...
    public function search( $params ) {
        ...
        $query->andFilterWhere('url', $this->url, true);
    ...
    public function getPageList($limit = 5) {
        return self::findAll(array('condition' => " type = '1' "));
    }
    ...
    public function getParents() {
        ...
        $models = Cms::findAllByAttributes(array('type' => self::PAGESET), $cond);
        $list = array();
    ...
    public function getBreadcrumbs($linkAll = false) {
        $pageId = $this->id;
        $bc = array('Site root' => array('cms/view'));
        $parents = Cms::asc()->findAll("rgt>={$this->rgt} AND lft<={$this->lft} AND parent_id!=0");
        if ($parents) {
            $bc = array();
            $cnt = count($parents);
        ...
        if ($parents) {
            ...
            foreach ($parents as $k => $p) {
                if (!$linkAll && $k + 1 == $cnt)
                    ...
                    $bc[$p->name] = array('cms/view', 'parent_id' => $p->id);
            }
            return $bc;
        }
        return array();
    ...
    function isTaken() {
        ...
        $count = Cms::count('url=:url AND type!=:type_link' . $idCond, $params);
        return $count;
    }
    ...
    function getImage() {
        ...
        return Html::image(Yii::$app->request->getBaseUrl() . '/images/cms/' . $images[$this->type], null, array('class' => 'drag'));
    }

    public static function getLevelMenu($attributes = array('parent_id' => 0)) {
        if ($items = self::findAllByAttributes($attributes)) {
            $menu = array();
