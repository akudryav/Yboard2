...
class JsTreeBehavior extends CBehavior {
    ...
    public function printULTree() {
        $categories = ActiveRecord::model($this->modelClassName)->findAll(array('order' => 'root,lft'));
        $level = 0;
        foreach ($categories as $n => $category) {
            ...
            echo Html::openTag('li', array('id' => 'node_' . $category->primaryKey, 'rel' => $category->getAttribute($this->rel_property)));
            echo Html::openTag('a', array('href' => '#'));
            echo Html::encode($category->getAttribute($this->label_property));
            echo Html::closeTag('a');
            echo "<div class='catActions'><a class='js_go' href='" . Yii::$app->createAbsoluteUrl("/view/category/update", array('id' => $category->primaryKey)
