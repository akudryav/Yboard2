...
class CmsHandler extends Controller {
    ...
    function initCms($root = false) {
        ...
        if (empty($url))
            $this->showError();
        if (!$root)
            $page = Cms::find('url=:url', array(':url' => $url));
        else
            $page = Cms::findOne(1);
        ...
        switch ($page->type) {
            case Cms::PAGESET:
                if ($page->overview_page || $page->id == 1) {
                    ...
                    $firstChild = Cms::find('parent_id=:parent_id', array(':parent_id' => $page->id));
                    if (!$firstChild) {
                        $this->showError();
                    }
    ...
    function renderPageContent($page) {
        if ($page->subsection) {
            $content = $this->render($page->subsection, array('page' => $page, 'content' => $page->content), true);
        }
        return $content;
    }
    ...
    function showError() {
        ...
        if ($error = Yii::$app->errorHandler->error) {
            if (Yii::$app->request->isAjaxRequest)
                ...
                return $this->render('//cms/error', array('error' => $error));
        }
    }
